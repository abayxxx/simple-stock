...
@section('title', 'Faktur Pembelian Belum Lunas per Supplier')

@section('content_header')
<h1>Faktur Pembelian - Belum Lunas per Supplier</h1>
@stop
...
<div class="mb-2 mt-3">
    <form class="form-inline" id="filter-form">
        ...
        <input type="text" name="supplier" class="form-control mr-1" placeholder="Cari Supplier" value="{{ request('supplier') }}">
...
<script>
    $(function() {
        let table = $('#unpaid-invoice-table').DataTable({
            ...
            searching: true,
            ajax: {
                url: '{{ route("purchases.unpaid.data") }}',
                data: function(d) {
                    d.from = $('[name="from"]').val();
                    d.to = $('[name="to"]').val();
                    d.supplier = $('[name="supplier"]').val();
                }
            },
            columns: [{
                    data: 'kode',
        ...
        function buildExportUrl() {
            ...
            return "{{ route('purchases.unpaid.export') }}" + "?from=" + from + "&to=" + to + "&supplier=" + supplier;
        }
        $('#export-btn').attr('href', buildExportUrl());
        $('[name=from], [name=to], [name=supplier]').on('change keyup', function() {
            $('#export-btn').attr('href', buildExportUrl());
        });
    });
</script>
