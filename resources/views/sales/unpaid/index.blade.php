...
$(function(){
    let table = $('#unpaid-invoice-table').DataTable({
        ...
        columns: [
            ...
            { data: 'total_bayar', name: 'total_bayar', className:'text-right' },
            { data: 'sisa_tagihan', name: 'sisa_tagihan', className:'text-right' },
        ],
        drawCallback: function(settings){
            // Grand total di tfoot
            let api = this.api();
            let data = api.rows({ page: 'current' }).data();
            let grand_gt = 0, grand_bayar = 0, grand_sisa = 0;
